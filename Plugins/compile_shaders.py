...
def compile_to_spv(filepath):
    ...
    re = run([f"{path}/../Subsystems/nosShaderCompiler/Binaries/tools/{platform.system()}/glslc", "-O", "-g", "-o",  f"{filepath}_.spv", filepath], stdout=stdout, stderr=stderr, universal_newlines=True)
    if re.returncode != 0:
