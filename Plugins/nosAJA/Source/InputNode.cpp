...
//std::string GetChannelListName(AJADevice& device) { return "nos.aja.ChannelList:" + device.GetDisplayName(); }

struct InputNodeContext : NodeContext
{
	InputNodeContext(const nosFbNode* node) : NodeContext(node), CurrentChannel(this)
	{
		AJADevice::Init();
		LoadNode(node);
	}
	...
	void OnNodeMenuRequested(const nosContextMenuRequest* request) override
	{
		...
		if (!devices.empty())
			items.push_back(nos::CreateContextMenuItemDirect(fbb, "Open Input Channel", 0, &devices));
		if (!items.empty())
			HandleEvent(CreateAppEvent(fbb,
									   nos::app::CreateAppContextMenuUpdateDirect(
										   fbb, &NodeId, request->pos(), request->instigator(), &items)));
	}
	...
	void OnMenuCommand(nosUUID itemID, uint32_t cmd) override
	{
		...
		std::string channelName = (isQuad ? GetQuadName(channel) : ("SingleLink " + std::to_string(channel + 1)));
		{
			...
			channelPin.device =
				std::make_unique<TDevice>(TDevice{{}, device->GetSerialNumber(), device->GetDisplayName()});
			channelPin.channel_name = channelName;
			channelPin.is_input = true;
		...
		{ // Quad mode setting
			if (isQuad && !QuadLinkModePinId)
			{
				...
				pins.push_back(nos::fb::CreatePinDirect(fbb,
														&id,
														(channelName + " Mode").c_str(),
														input ? "nos.aja.QuadLinkInputMode" : "nos.aja.QuadLinkMode",
		...
		if (!pins.empty())
			HandleEvent(CreateAppEvent(
				fbb, nos::CreatePartialNodeUpdateDirect(fbb, &NodeId, ClearFlags::NONE, &pinsToDelete, &pins)));
	}
...
nosResult RegisterInputNode(nosNodeFunctions* functions)
{
	NOS_BIND_NODE_CLASS(NOS_NAME_STATIC("nos.aja.Input"), InputNodeContext, functions)
