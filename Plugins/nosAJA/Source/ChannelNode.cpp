...
struct ChannelNodeContext : NodeContext
{
	...
	{
		AJADevice* outDev;
		if (AJADevice::GetAvailableDevice(IsInput, &outDev))
		{
			return outDev->GetDisplayName();
		}
		else if (AJADevice::Devices.size() > 0)
		{
			return AJADevice::Devices.begin()->second->GetDisplayName();
		}
		return "NONE";
	}
