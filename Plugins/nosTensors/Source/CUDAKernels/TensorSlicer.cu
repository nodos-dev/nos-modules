...
extern "C" {
	__global__ void SliceTensor(void* InData, int TotalSizeInBytes, int BytesPerElement, int Width, int Height, void* OutData) {
		int idx = blockIdx.x * blockDim.x + threadIdx.x;
		char* inData = reinterpret_cast<char*>(InData);
		char* outData = reinterpret_cast<char*>(OutData);
		idx = idx * BytesPerElement;
		if ((idx + Width*Height*2*BytesPerElement) < (TotalSizeInBytes)) {
