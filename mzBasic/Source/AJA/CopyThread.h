...
struct TrackSync 
{
	...
	void UpdateLastFrameID() // called from scheduler thread
	{
		...
		//if (true)
		{
			GServices.Log("Resetting aja " + Name(), "");
		}
	}
	void UpdateDropCount() // called from scheduler thread
	{
		...
		if (!dropCounts.empty())
		{
			...
			GServices.Log("Drop on " + Name() + " : " + std::to_string(totalDrops), "");
		}
		else
		{
			FramesSinceLastDrop++;
...
struct CopyThread : TrackSync
{
    ...
    fb::vec2u GetSuitableDispatchSize() const;
    fb::vec2u Extent() const;
    bool IsInput() const;
    void AJAInputProc();
    void AJAOutputProc();
