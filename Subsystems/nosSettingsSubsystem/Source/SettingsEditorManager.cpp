...
namespace nos::sys::settings {
	...
	void SettingsEditorManager::OnMessageFromEditor(uint64_t editorId, nosBuffer message) {
		...
		auto msg = flatbuffers::GetRoot<nos::sys::settings::editor::SettingsUpdateFromEditor>(message.Data);
		...
		if (ret == NOS_RESULT_SUCCESS) {
			...
			auto src = msg->entry()->data()->data();
			memcpy(&params.Buffer.Data, &src, sizeof(void*));
			params.Buffer.Size = msg->entry()->data()->size();
